...
> **Hình 5.2:** *Thiết kế chi tiết của Gói Progression (Quản lý tiến trình)*
...
classDiagram
    ...
    NPCKyleController "1" --> "1" RecordingPrompter : nhắc nhở
...
Các nhân vật hướng dẫn ảo đóng vai trò hỗ trợ đắc lực và là một phần không thể tách rời của ClassroomManager. Lớp NPCKyleController điều khiển giảng viên ảo Kyle thực hiện các hoạt ảnh vẫy tay chào khi người học bước vào phòng học, suy nghĩ khi người học thực hành và vỗ tay khích lệ khi hoàn thành bài học. Lớp NPCLobbyInstructorController điều khiển nhân vật hướng dẫn tại sảnh hành lang để chào đón và đưa ra lời giới thiệu ban đầu cho người học. Lớp RecordingPrompter quản lý việc hiển thị các gợi ý văn bản nhắc nhở thời gian thực giúp định hướng hoạt động cho người học.

#### b, Gói Gesture Recognition (Nhận dạng cử chỉ)

> **Hình 5.3:** *Thiết kế chi tiết của Gói Gesture Recognition (Nhận dạng cử chỉ)*
...
classDiagram
    ...
    ClassroomManager ..> GestureHub : lắng nghe sự kiện
    QuizManager ..> GestureHub : lắng nghe sự kiện
    GestureLocomotionProvider ..> GestureHub : lắng nghe sự kiện
```

Đây là gói nền tảng chịu trách nhiệm thu nhận dữ liệu khớp xương tay và thực hiện nhận dạng các cử chỉ tĩnh và động của người học để xuất bản sự kiện tương tác. Lớp GestureHub giữ vai trò trung tâm hoạt động như một bộ trung chuyển sự kiện, cung cấp sự kiện tĩnh OnGestureDetected để tất cả các thành phần khác có thể đăng ký lắng nghe và phản hồi khi có cử chỉ tương ứng. 
...
Lớp UnistrokeRecognizer cung cấp thuật toán nhận diện nét vẽ 2D Unistroke, tính toán điểm số khớp mẫu so với các quỹ đạo mẫu được lưu trữ. Lớp GestureTrigger cho phép kích hoạt các sự kiện cử chỉ khi bàn tay người học tương tác vật lý với các vùng không gian xác định.

#### c, Gói Interaction Locomotion (Tương tác và di chuyển)

> **Hình 5.4:** *Thiết kế chi tiết của Gói Interaction Locomotion (Tương tác và di chuyển)*

```mermaid
%%{init: {'theme': 'neutral'}}%%
classDiagram
    namespace Interaction_Locomotion_Package {
        class GestureLocomotionProvider
        class WristDashboardUI
        class WristFollower
        class UIFaceCamera
    ...
    WristDashboardUI "1" --> "1" WristFollower : đi theo cổ tay
    WristDashboardUI ..> ProgressManager : đọc điểm số
    GestureLocomotionProvider ..> GestureHub : lắng nghe cử chỉ di chuyển
```

Đây là gói xử lý giao diện người dùng thực tế ảo và các cơ chế tương tác vật lý, di chuyển trong không gian 3D. Lớp GestureLocomotionProvider chịu trách nhiệm di chuyển người học bằng cách lắng nghe cử chỉ chỉ tay trỏ của cả hai bàn tay hướng về phía trước để kích hoạt di chuyển mượt mà (Smooth Locomotion). 

Lớp WristDashboardUI quản lý giao diện đeo cổ tay HUD (Head-Up Display - HUD), tự động hiện/ẩn khi người học lật cổ tay hướng lên trên và cập nhật điểm số tiến trình. Lớp WristFollower điều khiển bảng giao diện di chuyển bám sát vị trí cổ tay của người học. 


#### d, Gói NPC (Nhân vật hướng dẫn)

Đây là gói chịu trách nhiệm điều khiển hành vi và hoạt ảnh của các nhân vật hướng dẫn ảo trong phòng học. Lớp NPCKyleController quản lý máy trạng thái Animator để điều khiển giảng viên ảo Kyle thực hiện các hoạt ảnh vẫy tay chào khi bước vào phòng học, suy nghĩ khi người học thực hành và vỗ tay khích lệ khi hoàn thành bài học. 

Lớp NPCLobbyInstructorController điều khiển nhân vật hướng dẫn tại sảnh hành lang để chào đón và đưa ra lời giới thiệu ban đầu cho người học. Lớp RecordingPrompter quản lý việc hiển thị các gợi ý văn bản nhắc nhở thời gian thực giúp định hướng hoạt động cho người học.
...
1. **GestureRecognition_Package:** Chứa toàn bộ logic nhận diện các cử chỉ tay tĩnh và động cùng bộ trung chuyển sự kiện. Gói này có tính mô-đun hóa cao, có thể tái sử dụng trực tiếp cho các dự án thực tế ảo khác có yêu cầu tương tác tay trần.
2. **Orchestration_Package:** Quản lý tiến trình học tập tổng thể, lưu trữ điểm số, trạng thái phòng học ảo, cơ chế thi cử trắc nghiệm và hoạt ảnh phản hồi của các nhân vật hướng dẫn ảo (bao gồm các lớp ProgressManager, ClassroomManager, QuizManager, NPCKyleController, NPCLobbyInstructorController, RecordingPrompter).
3. **InteractionLocomotion_Package:** Điều khiển cơ chế di chuyển rẽ hướng theo cử chỉ chỉ tay trỏ của người học và quản lý giao diện bảng đeo cổ tay.
...
| **Orchestration_Package**         |      9       |       1.092        |    47,2 KB     |
