...
graph TD
    ...
    style Progression fill:#fff,stroke:#333,stroke-width:1px;
    style AcademicData fill:#fff,stroke:#333,stroke-width:1px;
    style GestureCaptureRecognition fill:#fff,stroke:#333,stroke-width:1px;
```

Dưới đây là mô tả vai trò và ý nghĩa của các gói theo cách phân nhóm trên:
* **Presentation & Interaction (Gói trình diễn và tương tác):** Là gói bên ngoài, đóng vai trò giao tiếp trực tiếp với người học thông qua hiển thị đồ họa trực quan, giao diện tiến trình học trên cổ tay, hoạt ảnh phản hồi của nhân vật hướng dẫn ảo, và xử lý di chuyển thực tế ảo. Gói này phụ thuộc vào Gói quản lý tiến trình để lấy dữ liệu tiến độ và Gói thu nhận và nhận dạng cử chỉ để thực hiện di chuyển bằng cử chỉ tay.
* **AcademicData (Gói dữ liệu học thuật):** Là gói bên ngoài, đóng vai trò chứa cấu trúc lưu trữ tĩnh các từ vựng ký hiệu và danh sách câu hỏi kiểm tra, làm tầng dữ liệu cơ sở cho toàn bộ logic lõi.
* **Core (Lõi bài giảng):** Là trung tâm xử lý logic cốt lõi của ứng dụng, được bao bọc độc lập để chứa các gói chức năng chính bao gồm:
  - *Progression (Gói quản lý tiến trình):* Quản lý tiến trình học tập tổng thể, lưu trữ điểm số cao nhất của người học, quản lý vòng đời và trạng thái phòng học ảo (bao gồm cả việc nạp câu hỏi thi, kiểm tra cử chỉ đáp án và chấm điểm tự động). Gói này phụ thuộc vào Gói thu nhận và nhận dạng cử chỉ để tiếp nhận tín hiệu cử chỉ đầu vào và Gói dữ liệu học thuật để lấy nội dung câu hỏi bài học.
  - *Gesture Capture & Recognition (Gói thu nhận và nhận dạng cử chỉ):* Đóng vai trò là hạt nhân của bài giảng tương tác, chịu trách nhiệm thu nhận chuyển động bàn tay từ cảm biến camera của kính thực tế ảo và thực hiện nhận dạng các tư thế cử chỉ tĩnh hoặc động trong không gian, cung cấp sự kiện đầu vào trực tiếp cho Gói quản lý tiến trình. Gói này phụ thuộc vào Gói dữ liệu học thuật để lấy cấu hình các mẫu cử chỉ.

---

### 5.1.3 Thiết kế chi tiết gói

Biểu đồ lớp UML chi tiết dưới đây mô tả cấu trúc và mối quan hệ giữa các lớp cốt lõi thuộc hai gói quan trọng nhất trong hệ thống: Orchestration_Package và GestureRecognition_Package.
...
classDiagram
    ...
    %% Lớp QuizManager thuộc gói Orchestration
    class QuizManager {
...
Quá trình phát triển đã xây dựng thành công ứng dụng bài giảng tương tác ASL VR hoàn chỉnh dưới dạng tệp cài đặt chạy trực tiếp trên thiết bị kính. Mã nguồn được tổ chức chặt chẽ thành 4 gói chính như đã thiết kế ở phần thiết kế kiến trúc, mỗi gói đóng gói một nhóm chức năng chuyên biệt:
...
2. **Orchestration_Package:** Quản lý tiến trình học tập tổng thể, lưu trữ điểm số, trạng thái phòng học ảo và cơ chế thi cử trắc nghiệm (bao gồm cả lớp ClassroomManager và lớp QuizManager).
...
4. **NPC_Package:** Quản lý hoạt ảnh và máy trạng thái hành vi phản hồi của giảng viên ảo Kyle.
...
| **Orchestration_Package**         |      6       |        753         |    35,0 KB     |
