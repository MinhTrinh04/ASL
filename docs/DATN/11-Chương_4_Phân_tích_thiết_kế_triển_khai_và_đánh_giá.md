...
```mermaid
flowchart TD
    StartNode((●)) --> Lobby([Lobby])

    Lobby --> Classrooms([Classrooms])
    Classrooms --> PracticeQuiz([Practice & Quiz])
    PracticeQuiz --> Menu([Menu])

    Lobby --- Menu
    Lobby --> ExitGame([Exit Game])
    ExitGame --> EndNode((●))

    class StartNode,EndNode startEnd;
    class Lobby,Classrooms greenNode;
    class PracticeQuiz,Menu,ExitGame redNode;
```
